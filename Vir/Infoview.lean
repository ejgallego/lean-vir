...
private def expandReactWidgetCommand
  ...
  let propsIdent := mkIdent `infoviewDemoProps
...
`widgetSpec`, `mount`, `unmount`, `irPackage`, and `infoviewDemoProps`
declarations in that namespace. Cursor movement is delivered as new `Surface`
props to the same React root; only IR package revision changes reload the VIR
runtime service.
-/
