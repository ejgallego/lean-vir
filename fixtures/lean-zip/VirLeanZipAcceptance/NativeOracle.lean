...
private def largeRepeatedText : ByteArray :=
  repeatPattern "Lean VIR makes verified compression portable. ".toUTF8 32768
private def largeHeterogeneous : ByteArray := concatenateBytes #[
...
private def largeCompressionCases : Array CompressionCase := #[
  {
    ...
    input := largeRepeatedText
    levels := upperLevels
  },
  {
    name := "large-heterogeneous"
    input := largeHeterogeneous
    levels := upperLevels
  }
]
...
private def writeProfileStages (outputDir : System.FilePath) : IO (Array String) := do
  let corpus := "large-heterogeneous"
  let inputFile := s!"{corpus}.input.bin"
  let mut manifest := #[]
  for level in #[9, 10] do
    let tokens := lzMatchP largeHeterogeneous level.toUInt8
    let tokensFile := s!"{corpus}.level-{level}.tokens.bin"
    IO.FS.writeBinFile (outputDir / tokensFile) tokens.bytes
    manifest := manifest.push s!"profile-match\t{corpus}\t{level}\t{inputFile}\t{tokensFile}"
    let baseSize := (deflateRawBasePPrep largeHeterogeneous tokens).1
    manifest := manifest.push
      s!"profile-base\t{corpus}\t{level}\t{inputFile}\t{tokensFile}\t{baseSize}"
  for (kind, compressed) in #[
      ("fast", deflateDynamicBlocksOptimalFast largeHeterogeneous sharedTokChunk),
      ("exact", deflateDynamicBlocksOptimal largeHeterogeneous sharedTokChunk)
    ] do
    let outputFile := s!"{corpus}.optimal-{kind}.deflate.bin"
    IO.FS.writeBinFile (outputDir / outputFile) compressed
    manifest := manifest.push s!"profile-optimal\t{corpus}\t{kind}\t{inputFile}\t{outputFile}"
  return manifest

def run (args : List String) : IO Unit := do
  let (outputDir, includeProfile) ← match args with
    | [outputDir] => pure (outputDir, false)
    | [outputDir, "--profile"] => pure (outputDir, true)
    | _ => throw (IO.userError
        "usage: virLeanZipAcceptanceOracle <output-dir> [--profile]")
  ...
  let profileManifest ← if includeProfile then writeProfileStages outputDir else pure #[]
  IO.FS.writeFile (outputDir / "manifest.tsv") <|
    String.intercalate "\n"
      (compressionManifest ++ largeCompressionManifest ++ prescanManifest ++ profileManifest).toList ++ "\n"
