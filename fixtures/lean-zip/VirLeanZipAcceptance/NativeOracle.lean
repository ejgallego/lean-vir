...
private def writeCompressionCases (outputDir : System.FilePath) : IO (Array String) := do
  let mut manifest := #[]
  for testCase in compressionCases do
    ...
    for level in testCase.levels do
      ...
      manifest := manifest.push s!"compress\t{testCase.name}\t{level}\t{inputFile}\t{outputFile}"
  return manifest
...
def run (args : List String) : IO Unit := do
  ...
  let compressionManifest ← writeCompressionCases outputDir
  let prescanManifest ← writePrescanCases outputDir
  IO.FS.writeFile (outputDir / "manifest.tsv") <|
    String.intercalate "\n" (compressionManifest ++ prescanManifest).toList ++ "\n"
