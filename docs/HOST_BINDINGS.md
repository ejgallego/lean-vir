...
when there is no matching element. `Document.querySelectorAll` immediately
materializes the browser's native static `NodeList` as an owned Lean
`Array (Js Element)`. The temporary JavaScript collection is released after
the snapshot is built; each element handle then follows Lean reachability.
...
for missing selectors. Its low-level `querySelectorAll` binding returns a
static NodeList that the public Lean API materializes as an owned snapshot; use
