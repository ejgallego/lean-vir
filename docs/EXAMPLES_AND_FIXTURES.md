...
For a smaller loop, the [package guide](PACKAGES.md#generate-a-local-package)
covers explicit-root and public-definition generation, config-driven
`prepare:irpkg`, and [loading](PACKAGES.md#load-the-development-runner) a served
...
Use `vir.call(name, ...args)` from the [runtime API](JS_API.md#calls-and-manifest)
...
[generator map](GENERATE_PACKAGE.md#implementation-ownership) identifies the classifier,
...
[host-import contract](LEAN_VIR_LIBRARY.md#packages-and-host-imports) and bind
...
or interface diagnostics, use [generator troubleshooting](GENERATE_PACKAGE.md#troubleshooting);
...
[upstream boundary](UPSTREAM_BOUNDARY.md#explicit-limitations); the
