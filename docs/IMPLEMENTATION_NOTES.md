...
`wasm/upstream_shim/interface_codec.cpp` codec now carries only recursive type
descriptors/signatures, while `web/src/vir-runtime.js` lowers JavaScript values
to owned Lean objects before calling the upstream interpreter through a
package-local slot.
...
`wasm/upstream_shim/object_abi.cpp`; those helpers call the real constructors
...
use object arguments/results as well; compact codec use is limited to type
descriptor/signature metadata.
