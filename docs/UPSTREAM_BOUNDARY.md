...
opaque `leanObject` descriptor for generic LeanRef object handles. The shim
caches export signature summaries by package generation and slot, so compact
signature bytes are streamed once per loaded package rather than on every call.
