...
including parameterized instances, direct scalar wrappers, and inherited parent
fields, nullary inductive enums, and structural `Lean.Expr` values. For enums,
no per-demo code is needed: the generator records the constructor list in the
package manifest and `/dev.html` renders a select control from that manifest
