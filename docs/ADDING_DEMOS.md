...
The current generic interface covers primitive scalars, byte arrays, selected
homogeneous `Array`/`List` shapes, selected `Option`/`Prod` shapes, nullary
inductive enums, and structural `Lean.Expr` values. For enums, no per-demo code
is needed: the generator records the constructor list in the package manifest
and `/dev.html` renders a select control from that manifest after the package is
loaded.
