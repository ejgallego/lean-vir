...
react.node.resource	react	renderer	supported	Vir.React.Node;Vir.React.ElementType	vir-host-bindings	test:runtime;test:upstream	Native ReactNode text element Fragment and ElementType resources with Props.Entry props and JS-owned children resources supported	none
...
react.components	react	renderer	supported	Vir.React.Component;Vir.React.Hooks;Vir.React.ReducerState	vir-host-bindings	test:runtime;test:upstream	Lean function components with ReactM Node construction hook-backed renderComponent useState JS-shaped useReducer useRef resource-shaped useEffect and dependency-list useEffect support	broaden component API as examples require
