...
react.root	react	renderer	supported	Vir.React.Root	vir-host-bindings	test:runtime;test:upstream	faithful create renderNode and unmount plus Lean-composed createFromSelector mountFromSelector render and renderComponent	none
...
infoview.rpc.refs	infoview	rpc	partial	Vir.Infoview.ProofWidgetsRpc;Vir.Infoview.Surface	vir-host-bindings;vir-infoview-widget	test:infoview;test:infoview:browser	Real current-goal server methods return and resolve genuine WithRpcRef values through the official native RPC session; reference identity remains exact without a descriptor resolver or duplicate store	broaden expression/context construction beyond current goals plus edit and tactic APIs
...
proofwidgets.rpc	proofwidgets	compat	partial	Vir.Infoview.RpcSession;Vir.Js.Promise;Vir.ProofWidgets.Html;Vir.Infoview.ProofWidgetsRpc	vir-host-bindings;vir-infoview-widget	test:infoview;test:infoview:browser	Direct native RPC client renders real server data and round-trips genuine WithRpcRef values; options cancellation rejection and rerendering covered in Chromium; provisional descriptor protocol retired	upstream serialized Html interoperability and broader expression/context APIs
