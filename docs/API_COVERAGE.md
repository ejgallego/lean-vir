...
browser.element.lifecycle	browser	dom	partial	Vir.Browser.Document;Vir.Browser.Element;Vir.Browser.DOMTokenList	vir-host-bindings	test:runtime;test:upstream	createElement appendChild remove exact classList get/set exact DOMTokenList add/remove/toggle and style setProperty supported	additional DOMTokenList and style members only when a demo requires them
