...
infoview.rpc.session	infoview	rpc	partial	Vir.Infoview.RpcSession;Vir.Infoview.Surface;Vir.Js.Promise;Vir.Js.Function1	vir-host-bindings;vir-infoview-widget	test:infoview;test:runtime;check:bindings	Surface carries the exact position-specific RpcSessionAtPos object; RpcSession.call returns the exact native Promise and direct then/catch accept exact native functions while preserving request response and nested reference identity	add native request options and a real server_rpc_method browser fixture with cancellation rejection and Server.WithRpcRef coverage
...
proofwidgets.rpc	proofwidgets	compat	partial	Vir.Infoview.RpcSession;Vir.Js.Promise;Vir.ProofWidgets.Rpc;Vir.ProofWidgets.Html;Vir.Infoview.ProofWidgetsRpc	vir-host-bindings;vir-infoview-widget	test:infoview;test:runtime;test:upstream	The direct lane exposes the exact official RPC session Promise request and response values; the older WithRpcRef resolveRef and ExprWithCtx.save compatibility path remains provisional until the direct lane covers its real client	port one real server_rpc_method component then remove the duplicate resolver and reference store
