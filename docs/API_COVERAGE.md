...
browser.element.lifecycle	browser	dom	partial	Vir.Browser.Document;Vir.Browser.Element;Vir.Browser.DOMTokenList;Vir.Browser.ElementCSSInlineStyle;Vir.Browser.CSSStyleDeclaration	vir-host-bindings	test:runtime;test:upstream	createElement appendChild remove exact classList get/set exact DOMTokenList add/remove/toggle checked inline-style capability exact style get/set and exact CSSStyleDeclaration setProperty supported	additional DOMTokenList and CSSStyleDeclaration members only when a demo requires them
