...
infoview.rpc.refs	infoview	rpc	partial	Vir.Infoview.ProofWidgetsRpc;Vir.ProofWidgets.Rpc	vir-host-bindings;vir-infoview-widget	test:infoview;test:runtime;test:upstream	RpcRef descriptors cross proofwidgets.rpc.resolveRef and resolve through a snapshot-backed infoview RPC method with expression type context source position revision store-key and known-constant metadata	add elaborator-backed ExprWithCtx object storage and lifetime model
