...
react.components	react	renderer	supported	Vir.React.Component;Vir.React.Hooks;Vir.React.StateTuple;Vir.React.ReducerTuple	vir-host-bindings	test:runtime;test:upstream	Explicit same-ID component identity with nested keyed boundaries plus exact useState useReducer useRef useMemo useCallback useContext and native effect values; Lean callback/effect adapters remain separately named	add root options reducer initializer and context creation when required
