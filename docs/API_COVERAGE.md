...
infoview.rpc.refs	infoview	rpc	partial	Vir.ProofWidgets.Rpc	vir-host-bindings;vir-infoview-widget	test:runtime;test:upstream	RpcRef descriptors can cross the host boundary through proofwidgets.rpc.inspectRef and are normalized by virtual and browser hosts	add server-side reference resolver and lifetime model
...
proofwidgets.rpc	proofwidgets	compat	partial	Vir.ProofWidgets.Rpc;Vir.ProofWidgets.Html	vir-host-bindings;vir-infoview-widget	test:runtime;test:upstream	WithRpcRef and inspectRef support an InteractiveExpr-shaped demo with typed props and a host inspect action; edit RPC and tactic UI are still missing	add real reference resolution plus edit and tactic APIs
