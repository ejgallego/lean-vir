...
infoview.rpc.refs	infoview	rpc	partial	Vir.Infoview.ProofWidgetsRpc;Vir.ProofWidgets.Rpc;Vir.Infoview.Surface	vir-host-bindings;vir-infoview-widget	test:infoview;test:runtime;test:upstream	Surface.proofWidgetsExpr carries a server-owned WithRpcRef ExprWithCtx prop through a Js ServerRef host resource; proofwidgets.rpc.resolveRef can resolve that standard Lean.Server.WithRpcRef path or fall back to descriptor refs with source position revision store-key and known-constant metadata	add elaborator-backed ExprWithCtx construction plus edit and tactic APIs
...
proofwidgets.rpc	proofwidgets	compat	partial	Vir.ProofWidgets.Rpc;Vir.ProofWidgets.Html;Vir.Infoview.ProofWidgetsRpc	vir-host-bindings;vir-infoview-widget	test:infoview;test:runtime;test:upstream	WithRpcRef resolveRef and ExprWithCtx.save support InteractiveExpr-shaped demos; ReactProofWidget receives a live server-owned ExprWithCtx prop and updates component-owned React state from its resolve callback; edit RPC and tactic UI are still missing	add elaborator-backed ExprWithCtx construction plus edit and tactic APIs
