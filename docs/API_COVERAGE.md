...
core.interface.lean_ref	core	interface	supported	Vir.Js	vir-runtime;vir-host-bindings	test:runtime	LeanRef toJSL fromJSL retainJSL releaseJSL provide independent aliases; React owns stored aliases through replacement and unmount	none
core.interface.runtime_ref	core	interface	supported	Vir.Runtime	vir-runtime	test:runtime	RuntimeM.Ref new get set swap modify and modifyGet share Lean-owned state across retained callbacks; replaced values follow Lean reference counting	none
...
browser.document.query	browser	dom	supported	Vir.Browser.Document	vir-host-bindings	test:runtime;test:upstream	querySelector querySelectorAll querySelectorAllSnapshot createElement getTitle and setTitle supported; snapshots return owned Array (Js Element) values	none
browser.element.text_attr	browser	dom	supported	Vir.Browser.Element	vir-host-bindings	test:runtime;test:upstream	querySelector querySelectorAll querySelectorAllSnapshot getInnerHTML setInnerHTMLUnsafe getTextContent setTextContent getAttribute and setAttribute supported	none
...
browser.timers	browser	dom	supported	Vir.Browser.Timer	vir-host-bindings	test:runtime;test:upstream	setTimeout clearTimeout setInterval clearInterval supported including retained callback cleanup	none
...
react.root	react	renderer	supported	Vir.React.Root	vir-host-bindings	test:runtime;test:upstream	create createFromSelector mountFromSelector render renderComponent unmount plus selector-owned renderIntoSelector renderComponentIntoSelector unmountSelector supported	none
react.node.resource	react	renderer	supported	Vir.React.Node;Vir.React.ElementType	vir-host-bindings	test:runtime;test:upstream	Native ReactNode text element Fragment and ElementType resources with Props.Entry props refs and JS-owned children resources supported	none
...
react.components	react	renderer	supported	Vir.React.Component;Vir.React.Hooks;Vir.React.ReducerState	vir-host-bindings	test:runtime;test:upstream	Lean function components with ReactM Node construction hook-backed renderComponent useState JS-shaped useReducer useRef useMemo resource-shaped useEffect dependency-list useEffect and external ElementType smoke support	broaden component API as examples require
...
proofwidgets.html_subset	proofwidgets	compat	partial	Vir.ProofWidgets.Html;Vir.React.Node	vir-host-bindings;vir-infoview-widget	test:runtime;test:upstream	Shallow ProofWidgets-style Html facade over native ReactNode resources with text element child-bearing component attrs handlers b img hr style and combinator port of static Jsx.lean cases	add JSX syntax and broader Html/component parity as real ports require
proofwidgets.rpc	proofwidgets	compat	partial	Vir.ProofWidgets.Rpc;Vir.ProofWidgets.Html;Vir.Infoview.ProofWidgetsRpc	vir-host-bindings;vir-infoview-widget	test:infoview;test:runtime;test:upstream	WithRpcRef resolveRef and ExprWithCtx.save support InteractiveExpr-shaped demos; ReactProofWidget receives a live server-owned current-goal ExprWithCtx prop and updates component-owned React state from its resolve callback; edit RPC and tactic UI are still missing	broaden ExprWithCtx beyond current goals plus edit and tactic APIs
