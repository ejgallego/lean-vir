...
react.node.resource	react	renderer	supported	Vir.React.Node	vir-host-bindings	test:runtime;test:upstream	Native ReactNode text and element resources with optional key props handlers children supported	none
...
react.components	react	renderer	supported	Vir.React.Component	vir-host-bindings	test:runtime;test:upstream	Lean function components with ReactM Node construction and hook-backed renderComponent support	broaden component API as examples require
...
infoview.rpc.refs	infoview	rpc	partial	Vir.Infoview.ProofWidgetsRpc;Vir.ProofWidgets.Rpc	vir-host-bindings;vir-infoview-widget	test:infoview;test:runtime;test:upstream	RpcRef descriptors cross proofwidgets.rpc.resolveRef and can resolve through descriptor fallback or standard Lean.Server.WithRpcRef server-owned expression refs with source position revision store-key and known-constant metadata	add elaborator-backed ExprWithCtx construction and widget prop integration
...
proofwidgets.rpc	proofwidgets	compat	partial	Vir.ProofWidgets.Rpc;Vir.ProofWidgets.Html;Vir.Infoview.ProofWidgetsRpc	vir-host-bindings;vir-infoview-widget	test:infoview;test:runtime;test:upstream	WithRpcRef resolveRef and ExprWithCtx.save support an InteractiveExpr-shaped demo with component-owned React state updated from descriptor or server-owned expression ref callbacks; edit RPC and tactic UI are still missing	add elaborator-backed ExprWithCtx props plus edit and tactic APIs
