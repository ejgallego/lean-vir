...
generic `Lean.Vir.LeanRef.toJs` / `fromJs` / `release` object handles. On the
Lean side those handles are surfaced as `Lean.Vir.JSL α`, an alias that remains
distinct from JavaScript-shaped `Js α` resources.
