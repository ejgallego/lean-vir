...
`String` round trips, the runtime may use the primitive-lane resolved-call path
instead of constructing a compact byte payload. That is a transparent
optimization of the same package-owned signature path, not a separate
user-facing API.
