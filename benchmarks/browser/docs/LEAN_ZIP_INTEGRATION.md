...
The descriptor remains `queued` until all of these producer entry points are
available at immutable revisions:
...
Activation then requires package-local smokes, exact checksums, all declared
browser backends ready, independent inflation for every output, and exact
native-oracle bytes for all three Lean routes. Timing values are not an
activation gate and are not publication evidence on an uncontrolled host.
