...
Test the benchmark subtree installed below web/dist. The selection defaults to
PAGES_BENCHMARK_EXAMPLE/PAGES_BENCHMARK_VARIANT and then prettyM/default.`);
  process.exit(extra ? 1 : 0);
}

const example =
  requestedExample ?? process.env.PAGES_BENCHMARK_EXAMPLE ?? "prettyM";
const variant =
  requestedVariant ?? process.env.PAGES_BENCHMARK_VARIANT ?? "default";
