...
assert.equal(report.summary.declarations, 133);
assert.equal(report.summary.virJs, 120);
...
assert.equal(report.summary.declaredTargets, 133);
assert.equal(report.summary.provided, 133);
...
assert.equal(report.summary.publicTargetEdges, 2894);
assert.equal(report.summary.targetsReachedByPublicEntries, 133);
...
assert.equal(JSON.parse(dataMatch[1]).summary.provided, 133);
...
console.log("shipped bindings smoke ok: 120 vir_js + 13 explicit conversions, 133/133 provided");
