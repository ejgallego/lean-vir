...
if (cli.out === null) {
  ...
  if (existing.replace(/\r\n/g, "\n") !== rendered) {
    fail(`${relative(root, cli.out)} is stale; rerun the corresponding render step without --check`);
  }
...
function renderMarkdownReport(report) {
  const dependencies = report.typeScriptDependencies;
  const lines = [
    "<!-- Generated by scripts/render-type-anchors.mjs; edit the authored type-anchor inputs and regenerate this file. -->",
    "",
    ...
    ...(dependencies === undefined ? [] : [
      `Descriptor closure: ${dependencies.roots.length} roots, ${dependencies.included.length} included dependencies, ${dependencies.unresolved.length} unresolved.`,
      "",
    ]),
    "```vir-type-anchors",
    ...
    `diagnostic_error\t${report.diagnosticSummary?.error ?? 0}`,
    `diagnostic_warning\t${report.diagnosticSummary?.warning ?? 0}`,
    `diagnostic_info\t${report.diagnosticSummary?.info ?? 0}`,
    "```",
    "",
  ];
...
function renderHtmlReport(report) {
  ...
  const dependencySummary = report.typeScriptDependencies === undefined
    ? ""
    : `<p class="dependency-summary">Descriptor closure: ${report.typeScriptDependencies.roots.length} roots, ${report.typeScriptDependencies.included.length} included dependencies, ${report.typeScriptDependencies.unresolved.length} unresolved.</p>`;
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .dependency-summary { margin-top: 8px; color: var(--muted); }
...
<body>
  <main>
    ...
    <section class="summary" aria-label="Type anchor summary">
      ...
      ${summaryCard("errors", report.diagnosticSummary?.error ?? 0)}
      ${summaryCard("warnings", report.diagnosticSummary?.warning ?? 0)}
      ${summaryCard("info", report.diagnosticSummary?.info ?? 0)}
    </section>
...
function renderHtmlAnchor(result) {
  ...
  const notes = result.note === undefined
    ? ""
  ...
  return `      <article class="anchor" id="${escapeAttr(`type-anchor-${slug(result.id)}`)}" data-vir-type-anchor-hover="${escapeAttr(hoverText(result))}">
        ...
        <div class="meta">
          ...
          <span>Relation: <code>${escapeHtml(result.relation ?? "audit")}</code></span>
        </div>
...
function renderHtmlDiagnostics(diagnostics) {
  ...
  const items = diagnostics.map((item) =>
    `<li><code>${escapeHtml(`${item.severity}/${item.code}`)}</code>: ${escapeHtml(item.message)}</li>`).join("");
  return `<ul class="diagnostics" aria-label="Review diagnostics">${items}</ul>`;
}
...
function renderAnchor(result) {
  ...
  const diagnostics = (result.diagnostics ?? []).map((item) =>
    `<li><code>${escapeHtml(`${item.severity}/${item.code}`)}</code>: ${escapeHtml(item.message)}</li>`);
  return [
...
function sourceHref(source) {
  ...
  const file = source.url ?? source.path.split("/").pop();
