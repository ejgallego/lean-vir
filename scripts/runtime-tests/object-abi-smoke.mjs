...
function withCallLaneCounters(runtime, body) {
  ...
  const counters = { objectCalls: 0, legacyByteCalls: 0 };
  ...
  runtime.exports.vir_call_resolved = (...args) => {
    counters.legacyByteCalls++;
    throw new Error(`unexpected byte value call with ${args.length} arguments`);
  };
  try {
    body(counters);
    return counters;
...
assert.equal(objectCounters.legacyByteCalls, 0);
...
assert.equal(prettyCounters.legacyByteCalls, 0);
...
assert.equal(exprObjectCounters.legacyByteCalls, 0);
assert.deepEqual(runtime.call("Vir.Fixtures.ListOption.classifySum", 0), {
  kind: "inl",
  value: "10",
});
