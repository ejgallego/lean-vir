...
function buildReport(inventory, providers) {
  ...
  return {
    ...
    version: 1,
    generatedBy: "scripts/generate-shipped-bindings-report.mjs",
    analysis: {
...
function renderHtml(report) {
  ...
  const data = JSON.stringify(clientReport).replaceAll("<", "\\u003c");
  return `<!doctype html>
<!-- Generated by scripts/generate-shipped-bindings-report.mjs; regenerate instead of editing. -->
...
async function emit(path, contents, check) {
  if (check) {
    if ((await readFile(path, "utf8").catch(() => null)) !== contents) {
      fail(`${relative(root, path)} is stale; rerun the corresponding shipped-bindings generation step`);
    }
  } else {
...
const report = buildReport(inventory, collectProviders());
