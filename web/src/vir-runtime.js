...
export function createVirImports(module, overrides = {}, hostState = null) {
  ...
  if (hostState !== null) {
    ...
    imports.env.vir_js_call_objects = (slot, argvPtr, argc) =>
      hostState.callObjects(slot, argvPtr, argc);
...
export class VirRuntime {
  ...
  liftObjectFunction(type, obj, label) {
    const signature = functionSignatureBytes(type, label);
    const signaturePtr = this.allocBytes(signature);
    try {
...
function functionSignatureBytes(type, label) {
