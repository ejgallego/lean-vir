...
const packaged = spawnSync(
  process.execPath,
  ["scripts/packages/package-sdk-artifact.mjs"],
  {
    encoding: "utf8",
  },
);
...
async function extractSdk(tempDir) {
  const extracted = spawnSync(
    "tar",
    ["-xzf", "web/public/downloads/lean-vir-sdk.tar.gz", "-C", tempDir],
    { encoding: "utf8" },
  );
...
try {
  ...
  const modules = {};
  const reactDependentModules = new Set([
    "vir-react-host-bindings.js",
    "vir-react-dom-client.js",
  ]);
  for (const moduleName of SDK_JS_MODULES.filter(
    (name) => !reactDependentModules.has(name),
  )) {
  ...
  assert.equal(
    runtime.debugWasmUrlFor("./wasm/vir-upstream.wasm"),
...
try {
  ...
  const reactHostBindings = await import(
    pathToFileURL(join(jsDir, "vir-react-host-bindings.js"))
  );
