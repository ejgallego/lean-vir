...
const timedLeaseCall = firstOwnedLease.callTimed(leaseJsNat(5));
assert.equal(leaseJsNatValue(timedLeaseCall.value), 12n);
assert.equal(timedLeaseCall.timings.marshalMs >= 0, true);
assert.equal(timedLeaseCall.timings.executeMs >= 0, true);
assert.equal(timedLeaseCall.timings.decodeMs >= 0, true);
assert.equal(timedLeaseCall.timings.hostMs >= 0, true);
assert.equal(timedLeaseCall.timings.hostMs <= timedLeaseCall.timings.executeMs, true);
assert.equal(timedLeaseCall.timings.totalMs >= 0, true);
assert.deepEqual(callbackLeaseRuntime.hostState.callTimings, []);
const observedLeaseTimings = [];
callbackLeaseRuntime.setCallbackTimingObserver((timings) => {
  observedLeaseTimings.push(timings);
});
assert.equal(leaseJsNatValue(firstOwnedLease(leaseJsNat(5))), 12n);
assert.equal(observedLeaseTimings.length, 1);
assert.equal(observedLeaseTimings[0].hostMs <= observedLeaseTimings[0].executeMs, true);
callbackLeaseRuntime.setCallbackTimingObserver(null);
assert.equal(leaseJsNatValue(firstOwnedLease(leaseJsNat(5))), 12n);
assert.equal(observedLeaseTimings.length, 1);
assert.throws(() => callbackLeaseRuntime.setCallbackTimingObserver(true), /function or null/);
