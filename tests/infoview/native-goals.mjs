...
try {
  ...
  await click();
  await wait(120);
  let collapsedPinned = await snapshot();
  assert.equal((await disclosure("hover")).open, false, "case collapses while its popup is pinned");
  assert.equal(collapsedPinned.popup, true, "collapse preserves the mounted pinned portal");
  assert.equal(collapsedPinned.popupVisibility, "hidden", "hidden anchor hides its pinned portal");
  await click();
  await wait(120);
  pinned = await snapshot();
  assert.equal((await disclosure("hover")).open, true, "case expands with the pinned term intact");
  assert.equal(pinned.popupVisibility, "visible", "expanded anchor restores its pinned portal");
  assert.equal(pinned.pinned, "true", "collapse and expand preserve the pin state");
  assert.equal(pinned.requests, beforePin.requests, "collapse and expand do not refetch type information");
